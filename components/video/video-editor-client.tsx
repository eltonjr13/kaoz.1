...
export function VideoEditorClient() {
  ...
  return (
    <div className="flex min-h-0 flex-1 overflow-y-auto w-full flex-col bg-[#1A1301] text-[#F2F2F2]">
      ...
      {status && statusVariant && StatusIcon && isUrgentStatus && (
        <div
      ...
      {status && statusVariant && StatusIcon && !isUrgentStatus && (
        <div
          ...
          aria-live="polite"
        >
          <div className={`rounded-2xl border p-4 shadow-2xl ${statusVariant.dialogClass}`}>
            <div className="flex items-start gap-3">
              <span
                className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full ${statusVariant.iconClass}`}
              >
                <StatusIcon size={18} />
              </span>
              <div className="min-w-0 flex-1">
                <p className="text-sm font-bold">{statusVariant.label}</p>
                <p className="mt-1 text-sm leading-relaxed text-white/80">{status.text}</p>
              </div>
              <button
                type="button"
