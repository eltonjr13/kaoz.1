...
import { extractFeatures, scoreWithReadout } from "./readout.ts";
import type {
  ActivitySample,
  ConnectomeGeometry,
  ConnectomeNode,
...
let readoutWeights: Float32Array | null = null;
let readoutBias = 0;

async function init(): Promise<void> {
  const config = workerData as WorkerInit;
  ...
  const readoutRaw = JSON.parse(
    await import("node:fs/promises").then((fs) =>
      fs.readFile(path.join(config.readoutPath), "utf8")
    )
  ) as { weights: number[]; bias: number };
...
function rank(request: EngineWorkerRequest): EngineWorkerResponse {
  ...
  try {
    ...
    for (let row = 0; row < count; row++) {
      const vector = request.candidateVectors.subarray(
        row * dimension,
        (row + 1) * dimension
      );
...
function energy(vector: Float32Array): number {
  let sum = 0;
...
async function main(): Promise<void> {
  ...
  parentPort.on("message", (message: EngineWorkerRequest) => {
    if (message.kind !== "rank") return;
