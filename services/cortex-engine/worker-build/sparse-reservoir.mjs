 ...
 */
export function sparseMatVec(matrix, vector, out) {
    const { indptr, indices, weights } = matrix;
    if (out.length !== matrix.size) {
        throw new ReservoirError("dimension", `saída esperada de tamanho ${matrix.size}, recebida ${out.length}`);
    }
 ...
 */
export function buildProjection(nodeCount, inputDimension, seed, fanIn = 8) {
...
function rowsToMatrix(rows, size, columns) {
...
export function step(input) {
    ...
    for (let i = 0; i < size; i++) {
        ...
        if (!Number.isFinite(activated)) {
            throw new ReservoirError("nan", `ativação não finita no nó ${i}: ${activated}`);
        }
 ...
 */
export function runSteps(matrix, projection, params, inputVector, initialState, sampleActivity = false, sampleSize = 0) {
    if (inputVector.length !== projection.columns) {
        throw new ReservoirError("dimension", `entrada esperada de tamanho ${projection.columns}, recebida ${inputVector.length}`);
    }
    if (matrix.columns !== matrix.size) {
        throw new ReservoirError("dimension", `matriz recorrente não quadrada: ${matrix.size}x${matrix.columns}`);
    }
 ...
 */
export function sampleStep(state, stepIndex, limit) {
    const order = Array.from(state.keys()).sort((a, b) => Math.abs(state[b]) - Math.abs(state[a]) || a - b);
