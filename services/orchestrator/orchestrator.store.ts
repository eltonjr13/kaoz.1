...
export class OrchestratorStore {
  ...
  private async locked<T>(key: string, action: () => Promise<T>): Promise<T> { const previous = this.locks.get(key) || Promise.resolve(); let release!: () => void; const current = new Promise<void>((resolve) => { release = resolve; }); const queued = previous.then(() => current); this.locks.set(key, queued); await previous; try { return await action(); } finally { release(); if (this.locks.get(key) === queued) this.locks.delete(key); } }
  ...
  async saveRun(run: ExecutionRun) { const id = safeId(run.id); return this.locked(`run:${id}`, async () => { run.updatedAt = new Date().toISOString(); await atomicWrite(path.join(dirs.runs, `${id}.json`), run); return run; }); }
  async updateRun(id: string, update: (run: ExecutionRun) => void | Promise<void>) { const safe = safeId(id); return this.locked(`run:${safe}`, async () => { const run = await read<ExecutionRun>(path.join(dirs.runs, `${safe}.json`)); if (!run) return null; await update(run); run.updatedAt = new Date().toISOString(); await atomicWrite(path.join(dirs.runs, `${safe}.json`), run); return run; }); }
  ...
  async appendEvent(event: OrchestratorEvent) { const id = safeId(event.planId); const file = path.join(dirs.events, `${id}.json`); await this.locked(`events:${id}`, async () => { const events = await read<OrchestratorEvent[]>(file) || []; events.push(event); await atomicWrite(file, events.slice(-1000)); }); }
