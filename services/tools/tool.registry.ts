...
const nativeTools:KaozTool[]=[
 ...
 {id:"davinci-free:start-batch",name:"Iniciar edição do curso em lote",description:"Cria uma fila persistente e renderiza uma prévia por aula usando a identidade compartilhada do curso.",source:"native",inputSchema:{type:"object",required:["requestId","courseName"],properties:{requestId:{type:"string"},folderPath:{type:"string"},manifestId:{type:"string"},courseName:{type:"string"},style:{type:"string",enum:["subtle","balanced","dynamic"]},captionsEnabled:{type:"boolean"},musicPath:{type:"string"},musicDb:{type:"number"},useAgent:{type:"boolean"}}},effect:"write",approvalMode:"step",timeoutMs:30_000,enabled:true},
