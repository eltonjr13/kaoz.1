...
const videoExportProfileSchema={type:"object",properties:{resolution:{type:"string",enum:["720p","1080p","2k"]},fps:{type:"number",enum:[24,30,60]},bitrateMode:{type:"string",enum:["recommended","high","custom"]},bitrateKbps:{type:"number",minimum:2000,maximum:50000},videoEncoder:{type:"string",enum:["auto","cpu"]}}};
const nativeTools:KaozTool[]=[
 ...
 {id:"davinci-free:start-render-job",name:"Iniciar renderização de vídeo",description:"Enfileira proxy, trecho exato ou exportação final persistente e retorna imediatamente.",source:"native",inputSchema:{type:"object",required:["requestId","planId","kind"],properties:{requestId:{type:"string"},planId:{type:"string"},kind:{type:"string",enum:["proxy","spot-preview","export","batch-export"]},destinationDirectory:{type:"string"},outputName:{type:"string"},startSeconds:{type:"number"},durationSeconds:{type:"number"},exportProfile:videoExportProfileSchema}},effect:"write",approvalMode:"step",timeoutMs:15_000,enabled:true},
 {id:"davinci-free:get-render-job",name:"Consultar renderização de vídeo",description:"Consulta progresso, etapa, ETA e resultado de um job persistente.",source:"native",inputSchema:{type:"object",required:["jobId"],properties:{jobId:{type:"string"}}},effect:"read",approvalMode:"never",timeoutMs:5_000,enabled:true},
 {id:"davinci-free:list-render-jobs",name:"Listar renderizações de vídeo",description:"Lista os jobs de vídeo, opcionalmente filtrados por plano.",source:"native",inputSchema:{type:"object",properties:{planId:{type:"string"}}},effect:"read",approvalMode:"never",timeoutMs:5_000,enabled:true},
 {id:"davinci-free:cancel-render-job",name:"Cancelar renderização de vídeo",description:"Cancela um job e remove a saída parcial sem apagar exportações concluídas.",source:"native",inputSchema:{type:"object",required:["jobId"],properties:{jobId:{type:"string"}}},effect:"write",approvalMode:"step",timeoutMs:10_000,enabled:true},
 {id:"davinci-free:resume-render-job",name:"Retomar renderização de vídeo",description:"Recoloca um job cancelado ou com falha na fila persistente.",source:"native",inputSchema:{type:"object",required:["jobId"],properties:{jobId:{type:"string"}}},effect:"write",approvalMode:"step",timeoutMs:10_000,enabled:true},
 {id:"davinci-free:get-render-settings",name:"Consultar cache de vídeo",description:"Lê pasta e limite do cache de renderização.",source:"native",inputSchema:{type:"object"},effect:"read",approvalMode:"never",timeoutMs:5_000,enabled:true},
 {id:"davinci-free:save-render-settings",name:"Configurar cache de vídeo",description:"Salva a pasta e o limite do cache pesado de vídeo.",source:"native",inputSchema:{type:"object",properties:{cacheDirectory:{type:"string"},cacheBudgetGb:{type:"number",enum:[5,20,50,100]}}},effect:"write",approvalMode:"step",timeoutMs:10_000,enabled:true},
 {id:"davinci-free:approve-intelligent",name:"Enviar exportação ao Resolve",description:"Prepara um plano de uso único no Resolve Free a partir de uma exportação final concluída; clientes antigos ainda podem pedir a renderização compatível.",source:"native",inputSchema:{type:"object",required:["requestId","planId"],properties:{requestId:{type:"string"},planId:{type:"string"},finalPath:{type:"string"},outputResolution:{type:"string",enum:["full-hd","source"]},videoEncoder:{type:"string",enum:["auto","cpu"]}}},effect:"external",approvalMode:"step",timeoutMs:900_000,enabled:true},
 ...
 {id:"davinci-free:start-batch",name:"Iniciar edição do curso em lote",description:"Cria uma fila persistente e envia as exportações finais de cada aula ao motor compartilhado.",source:"native",inputSchema:{type:"object",required:["requestId","courseName"],properties:{requestId:{type:"string"},folderPath:{type:"string"},downloadFolder:{type:"string"},manifestId:{type:"string"},courseName:{type:"string"},style:{type:"string",enum:["subtle","balanced","dynamic","meme"]},motionPace:{type:"string",enum:["calm","natural","energetic"]},captionsEnabled:{type:"boolean"},musicPath:{type:"string"},musicDb:{type:"number"},sfxEnabled:{type:"boolean"},sfxVolumeDb:{type:"number"},sfxPack:{type:"string",enum:["minimal","dynamic","tech"]},useAgent:{type:"boolean"},outputResolution:{type:"string",enum:["full-hd","source"]},videoEncoder:{type:"string",enum:["auto","cpu"]},exportProfile:videoExportProfileSchema,transcriptionRuntime:{type:"string",enum:["web","desktop"]},transcriptionMode:{type:"string",enum:["cloud","local"]},transcriptionModelId:{type:"string"},transcriptionDevice:{type:"string",enum:["auto","vulkan","cpu"]},transcriptionAllowCloudFallback:{type:"boolean"},selectedItemIds:{type:"array",items:{type:"string"}},selectedRelativePaths:{type:"array",items:{type:"string"}}}},effect:"write",approvalMode:"step",timeoutMs:30_000,enabled:true},
