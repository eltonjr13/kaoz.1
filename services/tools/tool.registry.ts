...
const nativeTools:KaozTool[]=[
 ...
 {id:"davinci-free:analyze-intelligent",name:"Analisar aula por áudio",description:"Segmenta e transcreve o áudio localmente, cria uma identidade visual persistente por curso e prepara as decisões semânticas da aula.",source:"native",inputSchema:{type:"object",required:["requestId","sourcePath","courseName","moduleName"],properties:{requestId:{type:"string"},sourcePath:{type:"string"},courseName:{type:"string"},moduleName:{type:"string"},style:{type:"string",enum:["subtle","balanced","dynamic"]},captionsEnabled:{type:"boolean"},reuseCourseTheme:{type:"boolean"},musicPath:{type:"string"},musicDb:{type:"number"},useAgent:{type:"boolean"},transcriptionRuntime:{type:"string",enum:["web","desktop"]}}},effect:"write",approvalMode:"step",timeoutMs:600_000,enabled:true},
 ...
 {id:"davinci-free:start-batch",name:"Iniciar edição do curso em lote",description:"Cria uma fila persistente e renderiza uma prévia por aula usando a identidade compartilhada do curso.",source:"native",inputSchema:{type:"object",required:["requestId","courseName"],properties:{requestId:{type:"string"},folderPath:{type:"string"},downloadFolder:{type:"string"},manifestId:{type:"string"},courseName:{type:"string"},style:{type:"string",enum:["subtle","balanced","dynamic"]},captionsEnabled:{type:"boolean"},musicPath:{type:"string"},musicDb:{type:"number"},useAgent:{type:"boolean"},outputResolution:{type:"string",enum:["full-hd","source"]},videoEncoder:{type:"string",enum:["auto","cpu"]},selectedItemIds:{type:"array",items:{type:"string"}},selectedRelativePaths:{type:"array",items:{type:"string"}}}},effect:"write",approvalMode:"step",timeoutMs:30_000,enabled:true},
