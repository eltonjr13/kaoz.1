    ...
    for (const jobRecord of jobs) {
      ...
      (async () => {
        try {
          ...
          console.log(`[Polling Worker] Baixando arquivo de lipsync de: ${jobRecord.lip_sync_video_path}`);
          const ext = path.extname(jobRecord.lip_sync_video_path) || ".mp4";
          ...
          const { data: lipsyncFileData, error: downloadLipsyncErr } = await supabase.storage
            .from("job-assets")
            .download(jobRecord.lip_sync_video_path);
